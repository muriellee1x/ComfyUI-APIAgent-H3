...
## Low-Price Profile Override

This section overrides every later environment, scene, composition, full-body-sheet, and multishot pattern for the `image-low-coin-gift-prompt-director` profile according to the parsed gift price.

- For 99–499, use an isolated main subject or centered subject group on one continuous, uniform, texture-free pure black background.
- For 99–499, do not select an environment route and do not begin either prompt with `环境为`, `环境是`, `environment is`, or equivalent environment wording. Do not preserve architecture, gardens, stages, streets, distant views, horizons, weather, scene space, or environmental light sources from user input or reference images.
- For 500–999, a pure black background remains valid but is not mandatory. The route may use one compact, simple, theme-supporting background with limited depth and decoration, such as a single display platform, a restrained interior corner, a small patch of plants, one light curtain, or one decorated surface.
- For 500–999, reject complex architecture, grand stages, panoramic cityscapes, deep horizons, multiple locations, crowded background groups, or multilayer environmental storytelling. Keep the background subordinate to the main subject.
- Keep the subject or subject group centered throughout 99–999. Do not use off-center composition and do not reduce it to a small background element.
- For 99–499, preserve only subject identity, styling, materials, colors, and necessary local props from user input or reference images. For 500–999, also preserve only the minimum background information needed to support the theme.
- For 99–499, the Chinese prompt must contain the exact clauses `主体位于画面中央` and `画面为纯黑色背景`; the English prompt must contain `main subject centered in the frame` and `pure black background`.
- For 500–999, both prompts must retain the corresponding centered-subject clause, but must not be forced to mention a black background.
- Apply the matching price-band override even when a later route table or example conflicts with it.
